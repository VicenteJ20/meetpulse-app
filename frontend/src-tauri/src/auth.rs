...
fn refresh(mut session: GoogleSession, client_id: &str, client_secret: Option<String>) -> Result<GoogleSession, String> { let mut params = vec![("client_id", client_id.to_string()), ("refresh_token", session.refresh_token.clone()), ("grant_type", "refresh_token".to_string())]; if let Some(secret) = client_secret.filter(|value| !value.is_empty()) { params.push(("client_secret", secret)); } let token = exchange(params)?; session.id_token = token.id_token; session.expires_at = now() + token.expires_in.saturating_sub(30); session.client_id = client_id.to_string(); let (email, name) = claims(&session.id_token)?; session.email = email; session.name = name; save(&session)?; Ok(session) }

#[tauri::command]
pub fn get_google_auth_token(client_id: String, client_secret: Option<String>) -> Result<Option<GoogleSession>, String> { let Some(session) = load()? else { return Ok(None) }; if session.expires_at > now() { return Ok(Some(session)); } if client_id.is_empty() { return Err("Google OAuth client ID is not configured".into()); } refresh(session, &client_id, client_secret).map(Some) }

#[tauri::command]
pub fn sign_in_with_google(client_id: String, client_secret: Option<String>) -> Result<GoogleSession, String> {
  ...
  let state = Uuid::new_v4().to_string(); let verifier = Uuid::new_v4().simple().to_string() + &Uuid::new_v4().simple().to_string();
  ...
  let (mut stream, _) = listener.accept().map_err(|e| e.to_string())?; let mut request = [0; 8192]; let size = stream.read(&mut request).map_err(|e| e.to_string())?; let first = String::from_utf8_lossy(&request[..size]); let path = first.split_whitespace().nth(1).ok_or("Invalid OAuth callback")?; let callback = Url::parse(&format!("http://localhost{path}")).map_err(|e| e.to_string())?;
  let pairs: std::collections::HashMap<_, _> = callback.query_pairs().into_owned().collect();
  let code = pairs.get("code").ok_or_else(|| pairs.get("error").cloned().unwrap_or_else(|| "Google login cancelled".to_string()))?; if pairs.get("state") != Some(&state) { return Err("Invalid OAuth state".into()); }
  ...
  match result {
    Ok(session) => { stream.write_all(b"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\nGoogle login completed. You can close this window and return to MeetPulse.").map_err(|e| e.to_string())?; Ok(session) }
    Err(error) => { let _ = stream.write_all(format!("HTTP/1.1 400 Bad Request\r\nContent-Type: text/html\r\n\r\nGoogle login failed: {error}").as_bytes()); Err(error) }
  }
}
#[tauri::command] pub fn sign_out_google() -> Result<(), String> { match entry()?.delete_credential() { Ok(()) | Err(keyring::Error::NoEntry) => Ok(()), Err(e) => Err(e.to_string()) } }
