...
export function ChunkProgressDisplay({
  ...
  const getChunkStatusColor = (status: ChunkStatus['status']) => {
    switch (status) {
      ...
      case 'processing':
        return 'text-blue-600 bg-brand/10 border-brand/25';
      case 'failed':
  ...
  return (
    <div className={`bg-card border border-border rounded-lg p-4 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          {!isPaused ? (
            <button
              onClick={onPause}
              className="bg-warning hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm transition-colors"
            ...
            <button
              onClick={onResume}
              className="bg-success hover:bg-green-600 text-white px-3 py-1 rounded text-sm transition-colors"
            >
              Resume
            </button>
          )}
          ...
          <button
            onClick={onCancel}
            className="bg-destructive hover:bg-red-600 text-white px-3 py-1 rounded text-sm transition-colors"
          >
            Cancel
          </button>
        </div>
      ...
      {progress.completed_chunks === progress.total_chunks && progress.total_chunks > 0 && (
        <div className="mt-4 bg-success/10 border border-success/25 rounded-lg p-3">
          <div className="flex items-center space-x-2">
