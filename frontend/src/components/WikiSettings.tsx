...
export function WikiSettings() {
  ...
  return <div className="max-w-2xl space-y-5 py-6">
    ...
    <label className="block text-sm font-medium">Wiki API URL<input value={config.baseUrl} onChange={e => setConfig({ ...config, baseUrl: e.target.value })} placeholder={DEFAULT_WIKI_CONFIG.baseUrl} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label>
    <p className="text-sm text-slate-600">Your Wiki tenant is assigned automatically after Google sign-in. Use the selector in Wiki to switch shared workspaces.</p>
